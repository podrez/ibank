...
export async function scrapeAccounts(page: Page): Promise<ScrapedAccount[]> {
  ...
  if (apiAccounts && apiAccounts.length > 0) {
    logger.info('[alfabank] Accounts fetched via API intercept', { count: apiAccounts.length });
    return apiAccounts;
  }

  logger.info('[alfabank] API intercept yielded nothing, falling back to DOM scraping');
...
function parseApiResponse(json: unknown): ScrapedAccount[] | null {
  ...
  if (Array.isArray(json)) {
    ...
    for (const key of ['accounts', 'Accounts', 'data', 'Data', 'items', 'Items', 'result', 'Result']) {
  ...
  for (const item of list) {
    ...
    const accountNumber = str(a, ['accountNumber', 'AccountNumber', 'account', 'Account', 'iban', 'Iban', 'number', 'Number']);
    const currency = str(a, ['currency', 'Currency', 'currencyCode', 'CurrencyCode', 'iso', 'Iso']);
    const balance = num(a, ['balance', 'Balance', 'amount', 'Amount', 'currentBalance', 'CurrentBalance']);
    const available = numOrNull(a, ['available', 'Available', 'availableBalance', 'AvailableBalance']);
    const name = str(a, ['name', 'Name', 'title', 'Title', 'description', 'Description', 'alias', 'Alias']);
...
async function domScrape(page: Page): Promise<ScrapedAccount[]> {
  ...
  if (process.env.DEBUG_SCREENSHOTS === 'true') {
    ...
    await page.screenshot({ path: `${dir}/alfabank-dashboard.png`, fullPage: true }).catch(() => null);
    logger.info('[alfabank] Dashboard snapshot saved to ./data/debug/');
  }
  ...
  for (const sel of cardSelectors) {
    ...
    logger.debug(`[alfabank] Found ${cards.length} potential account cards with: ${sel}`);
  ...
  throw new Error(
    '[alfabank] Could not find account data on the dashboard. ' +
    'Set DEBUG_SCREENSHOTS=true and inspect ./data/debug/alfabank-dashboard.html.',
  );
}

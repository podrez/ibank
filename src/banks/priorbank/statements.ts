...
async function domScrape(page: Page, req: StatementRequest): Promise<ScrapedTransaction[]> {
  ...
  const rawRows = await page.evaluate(() => {
    ...
    const out: Array<{date:string;docNumber:string;description:string;currency:string;unp:string;debit:string;credit:string}> = [];
    ...
    return out;
  }) as Array<{ date: string; docNumber: string; description: string; currency: string; unp: string; debit: string; credit: string }>;
